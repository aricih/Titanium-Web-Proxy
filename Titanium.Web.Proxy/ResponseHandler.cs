...
namespace Titanium.Web.Proxy
{
	...
	public partial class ProxyServer
	{
		...
		public async Task HandleHttpSessionResponse(SessionEventArgs args, CancellationToken cancellationToken = default(CancellationToken))
		{
			try
			{
				...
				if (args.WebSession.Response.HasBodyRead)
				{
					...
					{
						using (var compressedStream = await GetCompressedBody(contentEncoding, args.WebSession.Response.Body, cancellationToken: cancellationToken))
						{
		...
		private async Task<MemoryStream> GetCompressedBody(string encodingType, byte[] responseBodyStream, CancellationToken cancellationToken = default(CancellationToken))
		{
